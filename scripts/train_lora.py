...
def core_gpt_dataset_config_from_args(args):
    # Проверка наличия токена EOD в словаре токенизатора
    eod_id = None
    if _GLOBAL_TOKENIZER is not None:
        # Проверка наличия токена EOD
        eod_token = '<EOD>'
        if hasattr(_GLOBAL_TOKENIZER, 'vocab') and eod_token in _GLOBAL_TOKENIZER.vocab:
            eod_id = _GLOBAL_TOKENIZER.vocab[eod_token]
        else:
            # Попытка использовать альтернативный токен
            logger.warning(f"Token {eod_token} not found in tokenizer vocabulary!")
            if hasattr(_GLOBAL_TOKENIZER, 'eos_token_id') and _GLOBAL_TOKENIZER.eos_token_id is not None:
                logger.info(f"Using eos_token_id as EOD token: {_GLOBAL_TOKENIZER.eos_token_id}")
                eod_id = _GLOBAL_TOKENIZER.eos_token_id
            else:
                logger.warning("No suitable EOD token found! Using 0 as default.")
                eod_id = 0
    else:
...
def _build_tokenizer(args):
    ...
    try:
        ...
        if _GLOBAL_TOKENIZER is None:
            ...
            logger.info(f"Tokenizer loaded successfully. Vocab size: {len(_GLOBAL_TOKENIZER)}")
            
            # Проверка наличия токена EOD
            eod_token = '<EOD>'
            if hasattr(_GLOBAL_TOKENIZER, 'vocab') and eod_token in _GLOBAL_TOKENIZER.vocab:
                logger.info(f"EOD token found in vocabulary: {eod_token}")
            else:
                logger.warning(f"EOD token not found in vocabulary! Will use alternative.")
                
                # Вывод информации о доступных специальных токенах
                if hasattr(_GLOBAL_TOKENIZER, 'special_tokens_map'):
                    logger.info(f"Available special tokens: {_GLOBAL_TOKENIZER.special_tokens_map}")
